cc.Class({
    ...
    onLoad: function () {
        ...
        this.toolNode = this.node.getChildByName("ToolNode"); // 道具节点
        // this.cardJsonDict = {}; // 卡片字典
        // this.playerCardDict = {}; // 玩家卡片字典
        // this.loader.loadResWithCount("/config/Card", cc.JsonAsset, this.onJsonLoaded.bind(this));
        // this.playerCardDict = {}; // 玩家卡片字典
        ...
        this.magicPane.on("SellTool", (playerName, callBack) => { // 变卖道具
            this.scheduleOnce(() => {
                this.alertMgr.show("event", () => { // 弹出事件提示框
                    // let cards = this.playerCardDict[playerName];
                    // while (cards.length > 0) {
                    //     this.sellCard(playerName, cards[0]); // 迭代卖出所有道具
                    // }
                    callBack(); // 完成回调
                }, "SellTool", null, playerName);
            }, 0.5);
        });
    },
